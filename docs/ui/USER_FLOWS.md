...
| Начать пользоваться мобильным банком | Ссылка «Зарегистрироваться» | AUTH-02, контекст `existing_customer` |
| Открыть банковский продукт | Ссылка «Открыть счёт» | ACC-07 → AUTH-02, контекст `new_customer` |
...
flowchart LR
  ...
  AUTH01 -- "«Зарегистрироваться»: D2 = NO" --> AUTH02[AUTH-02 Register User]
...
- **Решено [D-16]:** «Зарегистрироваться» — отдельная ссылка с AUTH-01 на AUTH-02. Регистрация не проходит через ACC-07.
...
- **Решено [D-18]:** путь задаёт контекст онбординга: «Зарегистрироваться» — `existing_customer` (на схеме D1 = YES, D2 = NO), «Открыть счёт» — `new_customer` (D1 = NO). Контекст хранится во флоу регистрации, а не в компонентах.
- **Решено [D-15]:** поля шагов:
  ...
  - AUTH-05, только в контексте `existing_customer` — номер и срок действия карты [D-18]. Новому клиенту карточные поля не показываются: карты у него ещё нет.
...
- **Решено [D-18]:** «Открыть счёт» сейчас означает начало отношений с банком: ветка ведёт в регистрацию, конкретный банковский продукт не открывается и не придумывается. Карточные поля на AUTH-05 новому клиенту не показываются.
